...
static void KernelReadDBATs(bat_table_t * table)
{
    ...
    /* *************************************************************************
     * eieio - Ensure all load and store instructions finish because we are
     * going to be modifying the BAT.
     * isync - Wait for the above instruction to finish.
     * ************************************************************************/
    asm volatile("eieio; isync");
...
{
    // Assign 1 so that this variable gets into the retained .data section and
    // not .bss
    static uint8_t ucSyscallsSetupRequired = 1;
    // If the Syscalls have already been setup, don't do it again.
    if(!ucSyscallsSetupRequired)
        return;

    ucSyscallsSetupRequired = 0;

    // Write our own KernelReadDBATs function pointer to each of the Syscall
    // tables at ID 3600.
    // TODO: find out what the * 4 means
    ...
    // Write our own KernelWriteDBATs function pointer to each of the Syscall
    // tables at ID 3700.
    // TODO: find out what the * 4 means
    ...
    // The PrepareTitle function is used to setup game titles before launching.
    // 0x48000003 - In background app memory
    // my_PrepareTitle_hook is our ASM routine
    u32 addr_my_PrepareTitle_hook = ((u32)my_PrepareTitle_hook) | 0x48000003;
    // Finish up all cache operations at the location of the function
    DCFlushRange(&addr_my_PrepareTitle_hook, sizeof(addr_my_PrepareTitle_hook));

    // unsigned int addr, unsigned int src, unsigned int len
    // Copy the original instruction at the address to .data.
    SC0x25_KernelCopyData((u32)&origPrepareTitleInstr, (u32)addr_PrepareTitle_hook, 4);
    // Copy the hook instruction to the address
